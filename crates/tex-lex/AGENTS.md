...
- `src/lib.rs`: input sources, line normalization, frozen editor layout cursors with contiguous root-piece coverage, source-frame lexer state, tokenization, input stack, token-list replay, condition frames, canonical alignment brace state, and resumable summaries.
