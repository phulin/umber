...
impl<'a> CommandProcessor<'a> {
    ...
    /// Opens the count/group publication sidecar used by the fused command
    /// episode. Semantic values remain in the same aggregate banks read by
    /// ordinary command processing.
    pub(crate) fn begin_count_group_episode(
        &mut self,
    ) -> Result<tex_state::CountGroupEpisode, tex_state::CountGroupEpisodeBarrier> {
        self.state.begin_count_group_episode()
    }

    pub(crate) fn episode_count(&self, episode: &tex_state::CountGroupEpisode, index: u8) -> i32 {
        self.state.episode_count(episode, index)
    }

    pub(crate) fn episode_set_count(
        &mut self,
        episode: &mut tex_state::CountGroupEpisode,
        index: u8,
        value: i32,
        global: bool,
    ) {
        self.state.episode_set_count(episode, index, value, global);
    }

    pub(crate) fn episode_group_depth(&self, episode: &tex_state::CountGroupEpisode) -> u32 {
        self.state.episode_group_depth(episode)
    }

    pub(crate) fn episode_innermost_group_kind(
        &self,
        episode: &tex_state::CountGroupEpisode,
    ) -> Option<tex_state::GroupKind> {
        self.state.episode_innermost_group_kind(episode)
    }

    pub(crate) fn episode_enter_group(
        &mut self,
        episode: &mut tex_state::CountGroupEpisode,
        kind: tex_state::GroupKind,
    ) {
        self.state.episode_enter_group(episode, kind);
    }

    pub(crate) fn episode_leave_group(
        &mut self,
        episode: &mut tex_state::CountGroupEpisode,
        expected: tex_state::GroupKind,
    ) -> Result<(), tex_state::GroupMismatch> {
        self.state.episode_leave_group(episode, expected)
    }

    pub(crate) fn finish_count_group_episode(&mut self, episode: tex_state::CountGroupEpisode) {
        self.state.finish_count_group_episode(episode);
    }

    pub(crate) fn episode_has_pending_diagnostic(&self) -> bool {
        !self.command.semantic_diagnostics.is_empty()
    }

    pub(crate) fn episode_has_pending_file_framing(&self) -> bool {
        !self.command.file_framing_events.is_empty()
    }

    pub(crate) fn episode_effect_pos(&self) -> tex_state::EffectPos {
        self.state.episode_effect_pos()
    }

    pub(crate) fn episode_work(&mut self) -> crate::CommandWorkCounters {
        self.fuel.fuel_mut().work()
    }
