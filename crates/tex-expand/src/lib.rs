...
use tex_lex::{InputSource, InputStack, LexError, WorldInput};
...
use tex_state::{
    ...
    ParagraphBarrierReason, TokenListReplayKind, TracedExpansionToken, Universe,
};
...
pub use values::{
    append_token_selector_text, append_token_show_text, append_token_string_text,
...
pub use tex_state::{ResourceLookup, ResourceNeed, ResourceResult};
...
///
/// The 100-million-step default leaves substantial room above the measured
/// 3,323,945 productive steps used by the largest terminating LaTeX/PGF scan
/// in the external-document corpus. The limit is a runaway guard, not a TeX
/// semantic capacity: TeX.web §§372--373 keeps expanding until it delivers an
/// unexpandable token and has no analogous cumulative work counter.
pub const DEFAULT_EXPANSION_FUEL: u64 = 100_000_000;
