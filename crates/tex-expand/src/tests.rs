...
fn default_expansion_fuel_clears_the_measured_productive_workload_floor() {
    ...
    const EXPECTED_HEADROOM: u64 = 96_676_055;
    let expansion = ExpansionContext::new("texput");

    assert_eq!(crate::DEFAULT_EXPANSION_FUEL, 100_000_000);
    assert_eq!(
        crate::DEFAULT_EXPANSION_FUEL - MEASURED_PRODUCTIVE_WORKLOAD_MAX,
        EXPECTED_HEADROOM
    );
    assert_eq!(expansion.fuel_limit, 100_000_000);
    assert_eq!(expansion.remaining_fuel, 100_000_000);
}
...
fn bm_rebound_relax_conditional_recovery_uses_bounded_stack() {
    // Only the recovery loop is under a bounded stack; building the timeline
    // and installing primitives is ordinary setup and happens outside it.
    let mut stores = Universe::new_with_plain_catcodes();
    crate::install_expandable_primitives(&mut stores);
    crate::install_etex_expandable_primitives(&mut stores);
    stores.register_primitive_meaning("relax", Meaning::Relax);
