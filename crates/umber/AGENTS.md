...
- `tests/it/e2e_conformance.rs`: individually selectable Story, Gentle, and TRIP tests that exercise the complete pipeline and require strict final DVI identity through `parity-harness`; Story and Gentle are ignored external-reference tests, while TRIP runs conditionally when its two local inputs exist.
