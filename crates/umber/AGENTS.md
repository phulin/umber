...
- `src/main.rs`: `umber` binary entry point, CLI argument parsing, canonical `CommandState` source-tokenization for `lex-dump`, `expand-dump`/`run` dispatch, token formatting, profiling-only TeX82 memory-projection census publication (including failed bounded runs), accepted PDF font-closure receipt publication, and real-run file resolvers.
- `src/cli_resource.rs`: retained native project/cache/distribution resolution, the source/digest/offline-bounded authenticated manifest owner, cancellation-aware resource retries, incremental source replacement, finite engine fuel/step/frame/journal/effect configuration, accepted-run telemetry handoff, and identity-pinned PDF font-closure receipt projection.
...
- `src/watch.rs`: polling incremental watch driver, bounded distribution-owner reuse across replacement sessions, supersession/Ctrl-C cancellation, DVI publication, and phase latency reporting.
