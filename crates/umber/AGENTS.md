...
- `src/bin/gentle_profile.rs`: persistent optimized Gentle profiling runner with optional `profiling-stats` counters that preloads the external corpus into a shared in-memory World, repeats fresh engine sessions without per-iteration host staging, and measures a fixed 20%-position incremental edit with memo disabled/enabled and cold-DVI verification.
