...
- `tests/it/e2e_conformance.rs`: individually selectable Story, Gentle, TRIP, and e-TRIP tests that execute Umber in process against gitignored, locally generated `tests/corpus/e2e` DVI oracles through `parity-harness`; TRIP and e-TRIP share one two-phase format helper. Every case reaches its assets only through `assets::with_gate`, so none can skip silently. `e2e_conformance_story_canonical` additionally drives the same staged Story fixture through `CanonicalEngineSession` (not the legacy `EngineSession`) via a `StagedDirResourceHost`, byte-comparing against the same oracle to guard `umber2-johp`'s canonical/reference DVI parity milestone. The equivalent canonical Gentle gate is explicitly ignored and manual-only because full-document execution does not belong in routine native tests or the automated semantic tracer (see "Canonical Story and Gentle Regression Gates" in `docs/testing_infrastructure.md`).
