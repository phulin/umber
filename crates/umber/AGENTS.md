...
- `src/bin/gentle_profile.rs`: persistent optimized Gentle profiling runner with optional `profiling-stats` counters that preloads the external corpus into a shared in-memory World, repeats fresh engine sessions without per-iteration host staging, and separately enforces slow pagination-changing, cross-generation interaction, and fast suffix-adoption paths under memo disabled/enabled or explicit baseline/candidate policies with cold-DVI and named-boundary-schedule verification.
