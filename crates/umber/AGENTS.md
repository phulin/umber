...
- `src/bin/gentle_profile.rs`: persistent optimized Gentle profiling runner with optional `profiling-stats` counters that preloads the external corpus into a shared in-memory World, repeats fresh engine sessions without per-iteration host staging, and separately enforces slow pagination-changing, cross-generation interaction, fast suffix-adoption, and break-dependency hlist-rebreak paths under memo disabled/enabled or explicit baseline/candidate policies with cold-DVI, named-boundary-schedule, zero-import, and profiling-only state-hash journal-work verification.
