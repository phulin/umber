...
- `tests/it/resource_transitions.rs`: public native compile-session runner for the shared resource transition cases in `tests/resource-transition-cases.json`.
- `tests/it/cli.rs`: shared CLI integration helpers and one integration target's named behavior modules in `tests/it/cli/`. Those modules own entry/cache, bibliography, format publication, PDF publication, dump commands, committed corpus parity, identity/clock, recovery, resources, and execution limits.
- `tests/it/e2e_conformance.rs`: individually selectable Story, Gentle, TRIP, and e-TRIP gates against gitignored, locally generated `tests/corpus/e2e` DVI oracles. `tests/it/e2e_conformance/{trip_support,plain_support,phase_support,loaded_support}.rs` owns shared typed recipes, prepared jobs, parity comparison, and focused loaded-job helpers; the other files in that directory separate recipe checks, canonical DVI regressions, phase-channel contracts, loaded TRIP diagnostics, and loaded TRIP math cases. Every full-pipeline route prepares a complete Plain, raw TeX82, TRIP, or e-TRIP recipe through `PreparedFormatProvider`, then runs a fresh explicit `PreparedFormatJob`. Story and Gentle share Plain, the seven self-contained DVI regressions share raw TeX82, and TRIP/e-TRIP retain distinct typed recipes. Runtime controls prove finite guards, authenticated construction evidence, fresh loaded jobs, exact loaded channels and normalized DVI, plus advisory geometry. Direct format image/security tests remain separate, and full-document gates do not broaden the automated microfixture tracer (see "Canonical Story and Gentle Regression Gates" in `docs/testing_infrastructure.md`). Failing compatibility/parity assertions excluded from the command-core cutover closure gate retain explicit `#[ignore]` reasons and run manually with `cargo test -q -p umber --test it -- --ignored`; passing behavioral and parity checks remain routine.
