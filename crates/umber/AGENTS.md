...
- `src/bin/gentle_profile.rs`: persistent optimized Gentle profiling runner with optional `profiling-stats` counters that preloads the external corpus into a shared in-memory World, repeats fresh engine sessions without per-iteration host staging, and measures pagination-changing and height-preserving edits at a fixed 20%-position with memo disabled/enabled or explicit baseline/candidate policies, cold-DVI verification, and suffix-adoption enforcement.
