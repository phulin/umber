...
- `tests/it/e2e_conformance.rs`: individually selectable Story, Gentle, TRIP, and e-TRIP tests that execute Umber in process against gitignored, locally generated `tests/corpus/e2e` DVI oracles through `parity-harness`; TRIP and e-TRIP share one two-phase format helper. Every case reaches its assets only through `assets::with_gate`, so none can skip silently. `e2e_conformance_story_canonical` and `e2e_conformance_gentle_canonical` additionally drive the same staged fixtures through `CanonicalEngineSession` (not the legacy `EngineSession`) via a `StagedDirResourceHost`, byte-comparing against the same oracles to guard `umber2-johp`'s canonical/reference DVI parity milestone. Both are routine native tests. This full-document DVI gate does not weaken the separate automated differential-tracer boundary, which remains limited to committed microfixtures and synthetic fixtures (see "Canonical Story and Gentle Regression Gates" in `docs/testing_infrastructure.md`).
