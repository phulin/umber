...
- `src/bin/gentle_profile.rs`: persistent optimized Gentle profiling runner, gated by `profiling-stats`, that preloads the external corpus into a shared in-memory World and repeats fresh engine sessions without per-iteration host staging.
