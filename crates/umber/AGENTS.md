...
- `src/prepared_format.rs`: universal native persistent prepared-format provider, explicit job request, compatibility validation, and provider-owned fresh-memory World boundary.
...
- `tests/it/cli.rs`: integration tests for CLI success, usage errors, corpus dump output, committed diagnostic/DVI fixture parity, and metadata-driven closed native bibliography invocation cases.
- `tests/it/e2e_conformance.rs`: individually selectable Story, Gentle, TRIP, and e-TRIP tests that execute Umber in process against gitignored, locally generated `tests/corpus/e2e` DVI oracles through `parity-harness`; TRIP and e-TRIP share one two-phase format helper. Every case reaches its assets only through `assets::with_gate`, so none can skip silently. `e2e_conformance_story_canonical`, `e2e_conformance_gentle_canonical`, and the ignored `e2e_conformance_trip_canonical` probe drive staged fixtures through retained `CanonicalEngineSession`, retained `World` roots, and typed `StagedDirResourceHost` fulfillment without an `Executor`/`InputStack` fallback. Successful recipe-owned dump construction gates exact command streams and the structured format-image contract, retains advisory geometry diagnostics, and excludes terminal and log diagnostics; loaded and ordinary phases retain exact meaningful terminal, log, and normalized-DVI comparison. Its format probe separately excludes non-durable provenance, host, checkpoint, cache, and journal state and asserts registry reconstruction without overwriting loaded live meanings. Full-document gates do not weaken the separate automated differential-tracer boundary, which remains limited to committed microfixtures and synthetic fixtures (see "Canonical Story and Gentle Regression Gates" in `docs/testing_infrastructure.md`).
