...
- `src/main.rs`: `umber` binary entry point, CLI argument parsing, canonical `CommandState` source-tokenization for `lex-dump`, `expand-dump`/`run` dispatch, token formatting, accepted PDF font-closure receipt publication, and real-run file resolvers.
- `src/cli_resource.rs`: retained native project/cache/distribution resolution, cancellation-aware resource retries, incremental source replacement, finite engine fuel/step/frame/journal/effect configuration, accepted-run telemetry handoff, and identity-pinned PDF font-closure receipt projection.
