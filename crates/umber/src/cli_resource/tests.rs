...
fn retained_revision_does_not_refetch_resolved_distribution_file() {
    ...
    std::fs::write(objects.join(format!("ahash64-v1-{shard_digest}")), packed)
        .expect("index shard");
    let root = format!(
        "{{\"schema\":8,\"distribution\":\"watch-test\",\"objectsBaseUrl\":\"https://example.invalid/objects/\",\"shardBits\":0,\"shardCount\":1,\"shards\":[\"{shard_digest}\"]}}\n"
    );
    std::fs::write(distribution.join("manifest-v8.json"), root).expect("root manifest");
...
fn bounded_distribution_owner_reuses_verified_state_and_preserves_detection_boundaries() {
    ...
    let root_path = distribution.join("manifest-v8.json");
...
#[test]
fn verified_owner_retains_touched_packed_shards_and_replays_unseen_keys_offline() {
...

    {
        ...
        let shard = &loaded.shards[&0];
        assert!(shard.lookup("tex:first.sty").is_some());
        assert!(shard.lookup("tex:absent.pdf").is_none());
        assert!(shard.lookup("tex:later.sty").is_some());
    }
    ...
    let responses = resolver
        ...
        .expect("unseen key probes the retained packed shard offline")
        .responses;
    assert!(matches!(
        responses.as_slice(),
    ...
    assert_eq!(
        extended.retained_manifest_bytes,
        cold.retained_manifest_bytes
    );
    ...
    assert_eq!(warm.retained_manifest_bytes, cold.retained_manifest_bytes);
}
...
fn write_sharded_root(
    ...
    for (body, publish) in shards {
        let shard = ManifestShard::parse(body).expect("typed shard fixture");
        let bytes = pack_shard(&shard).expect("packed shard fixture");
    ...
    let quoted = digests
        ...
        .join(",");
    let root = format!(
        "{{\"schema\":8,\"distribution\":\"{distribution}\",\"objectsBaseUrl\":\"https://example.invalid/objects/\",\"shardBits\":{shard_bits},\"shardCount\":{},\"shards\":[{quoted}]}}\n",
        shards.len()
    )
    .into_bytes();
    std::fs::write(directory.join("manifest-v8.json"), &root).expect("root manifest");
    (root, digests)
}
...
fn exact_snapshot_delivers_corpus_tex_tfm_type1_and_vf_requests_offline() {
    ...
    for (kind, name, manifest_key) in requests {
        ...
        let verified = resolver
            ...
            .lock()
            .expect("verified distribution state");
        let loaded = verified.loaded.as_ref().expect("loaded snapshot");
        let index = shard_index_for_key(manifest_key, loaded.root.shard_bits)
            .expect("canonical snapshot shard");
        let entry = loaded.shards[&index]
            .lookup(manifest_key)
            .and_then(|record| record.file())
            .expect("packed record from canonical verified shard");
...
fn required_batch_after_declining_startup_hint<'store>(
    session: &mut VirtualCompileSession<'store>,
) -> NeedResources {
    let CompileAttemptResult::NeedResources(startup) = session.compile_attempt() else {
        panic!("startup hint round must precede the demanded input");
    };
    assert!(startup.required.is_empty());
    assert!(startup.probes.is_empty());
    assert!(!startup.prefetch_hints.is_empty());
    session
        .provide_resources(Vec::new())
        .expect("decline startup hints through the normal response contract");

    let CompileAttemptResult::NeedResources(required) = session.compile_attempt() else {
        panic!("declining startup hints must expose the demanded input");
    };
    assert!(!required.required.is_empty());
    required
}
...
fn loaded_format_replay_retains_admitted_input_bindings() {
    ...
    let objects = distribution.join("objects");
    std::fs::create_dir_all(&objects).expect("distribution objects directory");
    let engine = EngineMode::Latex;
    let closure_len = 4;
    let mut recipe = crate::FormatRecipe::raw_etex26();
    recipe.engine = engine;
    recipe.format_name = "du4r-probe".to_owned();
    recipe.format_ident_name = recipe.format_name.clone();
    recipe.distribution_identity = b"du4r-probe".to_vec();
    ...
    std::fs::write(distribution.join("manifest-v8.json"), root).expect("root manifest");
    let input = directory.path().join("main.tex");
    std::fs::write(&input, b"\\input du4r-00\n").expect("main input");
    let cancellation = FetchCancellation::new();
    let mut session = NativeCompileSession::new_with_cache(
        &NativeRunOptions {
            input,
...
fn format_closure_is_loaded_only_as_each_input_is_requested() {
    for (engine, closure_len) in [(EngineMode::Latex, 57), (EngineMode::PdfLatex, 60)] {
        ...
        let first = required_batch_after_declining_startup_hint(&mut session.session);
...
fn warm_root_shard_and_object_cache_resolve_offline() {
    ...
    let mut offline = DistributionResolver::new(
        cache,
        Some("https://example.invalid/manifest-v8.json".into()),
        Some(root_digest),
        true,
    );
