...
- `src/definition_arena.rs`: 8-byte compact generation-branded definition keys and
  ...
  journal since the checkpoint. A direct definition build retains its final
  origin while unpublished and seals it in the header's only write; published
  headers are immutable, so region rollback needs only the ordinary suffix
  boundary. This includes ancestor writes after a child exit and unequal group
  depths.
...
- `src/fork_arena.rs` and `src/fork_arena/tests.rs`: Safe caller-owned
  ...
  resolver prefixes, NodePool live-owner accounting plus warm exact-64-KiB
  vacancy reuse while stable slots retain incarnation-based stale rejection,
...
- `src/pure_memo.rs`: Optional entry/byte-bounded pure-query caches for pretolerance, page-breaking, and shipout results, bounded eviction telemetry, and explicit cache release.
