...
fn page_snapshot_clone_shares_roots_until_their_first_write() {
    ...
    assert!(Arc::ptr_eq(
        &page.current_page.chunks,
        &snapshot.current_page.chunks
    ));
...
#[test]
fn current_page_chunks_have_content_position_shape_and_share_full_prefixes() {
    let mut page = PageBuilderState::default();
    for value in 0..64 {
    ...
    assert_eq!(boundary.current_page.chunks.len(), 1);
    assert_eq!(page.current_page.chunks.len(), 2);
    assert!(Arc::ptr_eq(
        &boundary.current_page.chunks[0],
        &page.current_page.chunks[0]
    ));
