...
pub(super) fn validate_loaded_font_state(
    fonts: &[FormatFont],
    name_count: usize,
    env: &[FormatEnvEntry],
    ...
    let mut encoded_null = fonts[0].clone();
    ...
    for (raw, font) in fonts.iter().enumerate() {
        ...
        if font
            .identifier
            .is_some_and(|symbol| symbol as usize >= name_count)
        {
    ...
    if last_loaded_font as usize >= fonts.len() {
        return Err(StoreFormatError::Invalid("last loaded font is not live"));
    }

    let mut parameter_counts = vec![None; fonts.len()];
    ...
    for entry in env {
        ...
        match bank {
            ...
            crate::cell::BankTag::FontParamLen => {
                ...
                let count = usize::try_from(word)
                    .map_err(|_| StoreFormatError::Invalid("font parameter count exceeds usize"))?;
            ...
            crate::cell::BankTag::CurrentFont => {
                ...
                if word as u32 as usize >= fonts.len() {
                    return Err(StoreFormatError::Invalid("current font is not live"));
                }
                let symbol_plus_one = word >> 32;
                if symbol_plus_one != 0 && (symbol_plus_one - 1) as usize >= name_count {
                    return Err(StoreFormatError::Invalid(
            ...
            crate::cell::BankTag::MathFamilyFont => {
                ...
                if word > u64::from(u32::MAX) || word as u32 as usize >= fonts.len() {
    ...
    for (font, encoded) in fonts.iter().zip(&parameter_counts) {
