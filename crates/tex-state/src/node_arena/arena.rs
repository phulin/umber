...
impl NodeListBuilder {
    ...
    /// Registers one directly owned child while nested node fields still use
    /// their private compact construction coordinate.
    #[allow(
        dead_code,
        reason = "consumed by the nested-node migration after the owned substrate lands"
    )]
    pub(crate) fn own_child(&mut self, child: NodeListRef) -> NodeListId {
        let id = child.id();
        if !self
            .direct_children
            .iter()
