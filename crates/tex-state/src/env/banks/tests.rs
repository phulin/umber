...
fn dense_bank_reads_one_validated_slot() {
    ...
    bank.write(2, BankCell { value: 9, level: 3 })
        .expect("write");
    assert_eq!(bank.get(2).expect("read"), BankCell { value: 9, level: 3 });
...
fn absent_paged_values_are_direct_algorithmic_defaults() {
    ...
    bank.write(700, BankCell { value: 4, level: 2 })
        .expect("write");
    assert_eq!(bank.allocated_pages(), 1);
    assert_eq!(
        bank.get(700).expect("written"),
        BankCell { value: 4, level: 2 }
    );
    assert_eq!(
...
fn register_bank_keeps_classic_prefix_inline_and_pages_only_overflow() {
    ...
    bank.write(255, BankCell { value: 1, level: 1 })
        .expect("dense write");
    assert_eq!(bank.allocated_overflow_pages(), 0);
    bank.write(256, BankCell { value: 2, level: 1 })
