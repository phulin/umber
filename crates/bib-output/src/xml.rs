...
pub fn generate_xml_schema(
    ...
    sink.push("  </choice></zeroOrMore></element></define>\n</grammar>\n")?;
    sink.finish()
}
...
struct XmlWriter<'a, 'b> {
    sink: &'a mut OutputSink<'b>,
}
impl<'a, 'b> XmlWriter<'a, 'b> {
    fn new(sink: &'a mut OutputSink<'b>) -> Self {
        Self { sink }
    }
    ...
    fn line(&mut self, indent: usize, s: &str) -> Result<(), XmlOutputFailure> {
        self.sink.indented_line(indent, s)
    }
}
