...
- `src/test_state.rs`: test-only value projection over an explicitly separate
  node pool and page-material region, copied font/parameter values, and narrow
  borrowed typesetting traits; it is not a runtime or `Universe` facade.
...
- `src/linebreak/mod.rs`: line-breaking API, line-shape types, pass orchestration,
  ...
  settles discardable-run successor positions and widths from that same scalar
  prefix walk, and performs positional trace reads only under explicit trace
  demand. Active routes keep
...
- `src/linebreak/post.rs`: Borrowed-cursor post-line-break materialization for
  broken lines, skips, migrated discretionary material, and penalties.
  Production arena tapes use the executor's retained-range sink.
