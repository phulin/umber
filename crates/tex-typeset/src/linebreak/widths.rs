...
use tex_state::node_arena::{NodeCursor, NodeList, NodeRef, PackedNode};
...
pub(super) fn line_widths_nodes<S: TypesetState>(state: &S, nodes: &[Node]) -> Widths {
    line_widths_cursor(state, NodeCursor::owned(nodes), 0, nodes.len(), true)
}
