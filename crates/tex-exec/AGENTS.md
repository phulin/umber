...
- `src/align/`: alignment stomach sub-mode machinery; `preamble.rs` scans `\halign`/`\valign` preambles into frozen u/v templates, tabskip boundaries, and repeat metadata, with focused scanner tests in `preamble/tests.rs`; `execution.rs` runs the row/cell sub-mode and appends rows with TeX's live prev-depth spacing, with focused lifecycle, completion, and lookahead tests in `execution/tests.rs`; `template.rs` replays u/v templates and owns the span-time expansion interleave; `noalign.rs` scans `\noalign` groups on that same alignment list so vertical state changes reach the next row, with group-close tests in `noalign/tests.rs`; `packaging.rs` builds unset row/cell records and is the single place a span count reaches §797's quarterword field, so it carries §798's `if n>max_quarterword then confusion("256 spans")` guard for every packaging caller, with metric tests in `packaging/tests.rs`; `support.rs` holds alignment state accessors and token classifiers; `transitions.rs` is the typed command-core lifecycle/event seam with no raw token API; `widths.rs` orchestrates `fin_align`, with prototype/orchestration tests in `widths/tests.rs`, `widths/resolution.rs` and `widths/resolution/tests.rs` resolving and testing span-aware column widths, `widths/set.rs` and `widths/set/tests.rs` converting and testing unset rows/cells as ordinary set boxes, and `widths/debug.rs` checking that unset nodes do not escape.
