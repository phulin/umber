...
- `src/paragraph_memo.rs`: centralized fail-before-mutation accepted-history paragraph validation, exact-stamp and typed semantic dependency tiers, shared retained hlist/finished-line mounting, ordered count/integer-parameter redo, full source-transition checks, output-reachable provenance recipes and mounted rebinding, barrier classification, and telemetry.
