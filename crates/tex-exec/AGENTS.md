...
- `src/main_control.rs`: sole production command delivery and execution driver, including same-borrow expandable preflight, typed resource and nested-immediate continuations, snapshot-free ordinary/resource/effect/PDF/checkpoint episodes, the alignment/diagnostic-only compatibility retry adapter, complete-job versus fragment root completion, bounded terminal-input continuation, loaded-profile versus canonical engine-binary semantic selection, typed committed mutation/effect observations, strong macro owners across scanned assignment apply seams, the execution-owned memo capability, and direct tracked-region lifecycle and mode projection.
