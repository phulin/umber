...
- `src/align/`: alignment stomach sub-mode machinery; `preamble.rs` scans `\halign`/`\valign` preambles into frozen u/v templates, tabskip boundaries, and repeat metadata; `execution.rs` runs the row/cell sub-mode; `template.rs` replays u/v templates and owns the span-time expansion interleave; `noalign.rs` scans `\noalign` groups; `packaging.rs` builds unset row/cell records; `support.rs` holds alignment state accessors and token classifiers; `widths.rs` orchestrates `fin_align`, with `widths/resolution.rs` resolving span-aware column widths, `widths/set.rs` converting unset rows/cells to ordinary set boxes, and `widths/debug.rs` checking that unset nodes do not escape.
