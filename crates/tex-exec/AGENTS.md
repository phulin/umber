...
- `src/align/`: alignment stomach sub-mode machinery; `preamble.rs` scans `\halign`/`\valign` preambles into frozen u/v templates, tabskip boundaries, and repeat metadata, `execution.rs` runs the row/cell sub-mode and packages unset alignment records, and `widths.rs` resolves span-aware column widths and converts unset rows/cells to ordinary set boxes at `fin_align`.
