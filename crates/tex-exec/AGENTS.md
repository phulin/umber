...
- `src/align/`: alignment stomach sub-mode machinery; `preamble.rs` scans `\halign`/`\valign` preambles into frozen u/v templates, tabskip boundaries, and repeat metadata, with focused scanner tests in `preamble/tests.rs`; `execution.rs` runs the row/cell sub-mode and appends rows with TeX's live prev-depth spacing, with focused lifecycle and lookahead tests in `execution/tests.rs`; `template.rs` replays u/v templates and owns the span-time expansion interleave; `noalign.rs` scans `\noalign` groups on that same alignment list so vertical state changes reach the next row; `packaging.rs` builds unset row/cell records; `support.rs` holds alignment state accessors and token classifiers; `transitions.rs` is the typed command-core lifecycle/event seam with no raw token API; `widths.rs` orchestrates `fin_align`, with `widths/resolution.rs` resolving span-aware column widths, `widths/set.rs` converting unset rows/cells to ordinary set boxes, and `widths/debug.rs` checking that unset nodes do not escape.
