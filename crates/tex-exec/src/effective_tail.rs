...
impl<'a> EffectiveTail<'a> {
    ...
    where
        I: DoubleEndedIterator<Item = &'a Node> + ExactSizeIterator,
    {
        let len = nodes.len();
        let last = nodes.next_back()?;
        let (node, index) = if matches!(last, Node::Direction(Direction::EndM)) {
        ...
        let preceded_by_begin = nodes
            .next_back()
            .is_some_and(|node| matches!(node, Node::Direction(Direction::BeginM)));
        Some(Self {
            node,
            index,
            len,
...
mod tests {
    ...
    fn generated_end_m_is_transparent_but_real_math_off_is_not() {
        ...
        let tail = EffectiveTail::find(generated.iter()).expect("effective tail");
        assert!(matches!(tail.node(), Node::Kern { .. }));
        assert_eq!(tail.removal_range(), 0..=2);

        let math_off = [kern, Node::MathOff(Scaled::from_raw(0))];
        let tail = EffectiveTail::find(math_off.iter()).expect("effective tail");
        assert!(matches!(tail.node(), Node::MathOff(_)));
    ...
    fn unmatched_generated_end_m_survives_effective_tail_removal() {
        ...
        let tail = EffectiveTail::find(nodes.iter()).expect("effective tail");
        assert_eq!(tail.removal_range(), 1..=1);
    }
}
