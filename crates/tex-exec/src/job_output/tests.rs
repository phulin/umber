...
fn channel_text(stores: &Universe, sink: PrintSink) -> String {
    stores
        .world()
        .effect_records()
        .iter()
...
fn lazy_dvi_and_log_names_follow_job_name_and_texput_default() {
    for (job_name, stem) in [("paper", "paper"), ("", "texput"), ("texput", "texput")] {
        ...
        assert_eq!(
            output.dvi_name(&mut stores, job_name),
        ...
        assert_eq!(
            output.open_log(&mut stores, job_name),
...
fn output_and_transcript_open_retry_preserve_canonical_selector_behavior() {
    ...
    assert_eq!(
        output.dvi_name(&mut stores, "paper"),
        Ok("alternate-output.dvi")
    );
    assert_eq!(
        output.open_log(&mut stores, "paper"),
        Ok("alternate-transcript.log")
    );
    assert!(output.log_opened());
    let terminal = channel_text(&stores, PrintSink::Terminal);
    assert_eq!(
        terminal
    ...
    let log = channel_text(&stores, PrintSink::Log);
    assert!(log.contains("alternate-output\n"));
    assert!(log.contains("alternate-transcript.log\n"));

    let mut batch = Universe::new();
    batch.set_interaction_mode(InteractionMode::Batch);
    batch.world_mut().deny_memory_output("paper.log");
    let mut output = JobOutput::default();
    assert_eq!(
        output.open_log(&mut batch, "paper"),
        Err(JobOutputOpenError::NonInteractive)
    );
    assert!(!output.log_opened());
    assert!(channel_text(&batch, PrintSink::Terminal).contains("paper.log"));
}
