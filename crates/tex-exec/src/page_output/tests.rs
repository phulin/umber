...
fn earlier_break_preserves_unrelated_pending_penalty() {
    ...
    let penalty = output_penalty_and_rewrite_break(&mut stores, &mut after_break, fire_up(1, 2));
...
fn chosen_pending_penalty_is_rewritten() {
    ...
    let penalty = output_penalty_and_rewrite_break(&mut stores, &mut after_break, fire_up(1, 1));
