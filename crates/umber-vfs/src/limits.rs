...
impl VfsLimits {
    ...
    pub fn validate(self) -> Result<Self, VfsLimitError> {
        for (kind, attempted, hard) in [
            ...
            (
                VfsLimitKind::StageFiles,
                self.stage_files,
                Self::HARD_MAX.stage_files,
            ),
            (
                VfsLimitKind::GeneratedFiles,
                self.generated_files,
                Self::HARD_MAX.generated_files,
            ),
            (
            ...
            (
                ...
                self.stage_bytes,
                Self::HARD_MAX.stage_bytes,
            ),
            (
                VfsLimitKind::GeneratedBytes,
                self.generated_bytes,
                Self::HARD_MAX.generated_bytes,
            ),
        ] {
            if attempted > hard {
    ...
    pub fn check(&self, kind: VfsLimitKind, attempted: usize) -> Result<(), VfsLimitError> {
        let limit = match kind {
            ...
            VfsLimitKind::StageFiles => self.stage_files,
            VfsLimitKind::GeneratedFiles => self.generated_files,
            ...
            VfsLimitKind::StageBytes => self.stage_bytes,
            VfsLimitKind::GeneratedBytes => self.generated_bytes,
        };
        if attempted > limit {
...
impl VfsLimitKind {
    ...
    pub const fn description(self) -> &'static str {
        match self {
            ...
            Self::StageFiles => "stage generated files",
            Self::GeneratedFiles => "build generated files",
            Self::OneFileBytes => "one file bytes",
            Self::UserBytes => "user source bytes",
            Self::ResolvedBytes => "cached file bytes",
            Self::StageBytes => "stage generated bytes",
            Self::GeneratedBytes => "build generated bytes",
        }
    }
}
