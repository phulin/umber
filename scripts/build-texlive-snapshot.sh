...
logical_files="$(jq '.files | length' "$output_dir/manifest.json")"
...
printf 'TeX Live snapshot: logical_files=%s objects=%s bytes=%s manifest_sha256=%s output=%s\n' \
  "$logical_files" "$objects" "$bytes" "$manifest_digest" "$output_dir"
