...
impl Pdf {
    ...
    pub fn finish(self) -> Vec<u8> {
        ...
        let xref_len = write_xref_entries(entries, &mut writer);
    ...
    fn finish_with_xref_stream_inner(
        ...
        let xref_offset = buf.len();
        entries.push((xref_id, XRefEntry::InUse { offset: xref_offset }));
        let field_width = determine_xref_field_width(&entries);
...
fn write_xref_entries(
    mut entries: Vec<(Ref, XRefEntry)>,
    writer: &mut impl XRefWriter,
) -> i32 {
    entries.sort_by_key(|&(id, _)| id);

    let xref_len = 1 + entries.last().map_or(0, |p| p.0.get());
    writer.prologue(xref_len);

    if entries.is_empty() {
        writer.write_entry(XRefEntry::Free { next: 0, generation: 65535 });
    }

    let mut written = 0;
    for (i, (object_id, entry)) in entries.iter().enumerate() {
        ...
        for free_id in start..object_id.get() {
            ...
            writer.write_entry(XRefEntry::Free {
                next: (next % xref_len) as usize,
                generation: gen,
            });
            written += 1;
        }
...
trait XRefWriter {
    ...
    fn write_entry(&mut self, entry: XRefEntry) {
        match entry {
            XRefEntry::Free { next, generation } => {
                self.write_free_entry(next, generation)
            }
            XRefEntry::InUse { offset } => self.write_occupied_entry(offset, 0),
            XRefEntry::Compressed { container, index } => {
                self.write_compressed_entry(container, index)
            }
        }
    }
    fn write_free_entry(&mut self, offset: usize, gen_number: u16);
    fn write_occupied_entry(&mut self, offset: usize, gen_number: u16);
    fn write_compressed_entry(&mut self, container: Ref, index: u16);
}
...
impl XRefWriter for XRefStreamWriter {
    ...
    fn write_compressed_entry(&mut self, container: Ref, index: u16) {
        self.write(2, container.get() as usize, index);
    }
}
...
impl<'a> XRefWriter for PlainXRefWriter<'a> {
    ...
    fn write_compressed_entry(&mut self, _: Ref, _: u16) {
        panic!("compressed objects require a cross-reference stream");
    }
}

fn determine_xref_field_width(entries: &[(Ref, XRefEntry)]) -> u32 {
    let largest = entries
        .iter()
        .map(|&(_, entry)| match entry {
            XRefEntry::Free { next, .. } => next,
            XRefEntry::InUse { offset } => offset,
            XRefEntry::Compressed { container, .. } => container.get() as usize,
        })
        .max()
        .unwrap_or(0);
    determine_field_width(largest)
}

fn determine_field_width(value: usize) -> u32 {
    (usize::BITS - value.leading_zeros()).div_ceil(8)
}
...
mod tests {
    ...
    pub fn print_chunk(chunk: &Chunk) {
        println!("========== Chunk ==========");
        for &(id, entry) in &chunk.entries {
            println!("[{}]: {:?}", id.get(), entry);
        }
        println!("---------------------------");
