...
pub fn renumber(source: &Chunk, target: &mut Chunk, mapping: &mut dyn FnMut(Ref) -> Ref) {
    assert!(
        source
            .entries
            .iter()
            .all(|(_, entry)| matches!(entry, XRefEntry::InUse { .. })),
        "chunks containing object streams cannot be renumbered"
    );
    ...
    let mut iter = source
        ...
        .filter_map(|&(id, entry)| match entry {
            XRefEntry::InUse { offset } => Some((id, offset)),
            XRefEntry::Free { .. } | XRefEntry::Compressed { .. } => None,
        })
        .peekable();
    while let Some((id, offset)) = iter.next() {
        ...
        target
            .entries
            .push((new, XRefEntry::InUse { offset: target.buf.len() }));
...
mod tests {
    ...
    fn test_renumber() {
        ...
        c.entries
            .push((Ref::new(8), XRefEntry::InUse { offset: c.buf.len() }));
