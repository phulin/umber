...
case's complete channel block. In particular, `channels.events` counts Umber
observations at the fragment boundary; it is not an independently captured
oracle event count. Review the entire diff before accepting a projection, and
retain an event baseline whose change has not been adjudicated.
