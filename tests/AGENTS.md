...
run directory, runs `fixturegen --reference-run` against the live reference engine, and
...
regeneration builds and runs `fixturegen --reference-run`, copies the pinned `cmr10`,
...
DVI SHA-256 after the same banner-only normalization used by `parity-harness`.
