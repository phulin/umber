...
- `--area AREA` regenerates one area, such as `exec`, `typeset`, `dvi`,
  `page`, `math`, `align`, `leaders`, `tex_exec`, or `tex_exec_io`.
- `--case AREA/CASE` regenerates one DVI case exactly. Text areas still route
  through their owning focused test; unchanged fixtures are not rewritten.

The script requires a live reference TeX (`pdftex` or `tex` on `PATH`, or
`UMBER_REF_TEX=/absolute/path/to/pdftex`) for pdfTeX-derived text and DVI
fixtures. DVI regeneration builds and runs `tools/refexec`, copies the pinned
`cmr10`, `cmmi10`, `cmsy10`, and `cmex10` TFMs from
