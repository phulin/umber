...
`tests/corpus/command-semantic` contains the versioned schema and independently owned domain manifests for tiny property-scoped semantic inputs. `alignments/` projects the seven remaining alignment lifecycle and packing gaps as strict xfails, `conditionals/` projects conditional observations, `input-expansion/` projects filtered input and command observations with hermetic host responses, and `math/` projects selected command, mode, final-box, and committed shipout boundaries. The generic runner in `tools/tex-command-stream/tests/it/command_semantic.rs` discovers every domain without a Rust registry, validates catalogue ownership, source bounds, exact provenance, duplicates, and strict xfail fingerprints, then compares concise canonical-main-control projections. Cargo tests invoke no live TeX and never read the long-document trace registry.
