...
and the evaluation-only native batch-kernel ceiling. It is excluded from the
root workspace correctness gate.
...
`native_batch` compares production `MainControl` with a direct mutable kernel
...
python3 scripts/run-umber-guarded.py --timeout-seconds 600 --max-rss-mib 4096 -- benchmarks/tex-exec/target/release/native_batch fused 89551 10 direct
...
and serialize DVI, and retain terminal text, log text, and effects. The direct
kernel is deliberately not a supported engine choice; its architecture,
measurements, and deletion-oriented migration plan are recorded in
