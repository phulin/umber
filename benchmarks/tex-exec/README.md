...
32 requested checkpoint operations. Each row measures capture, fixed
accounting-field copy, fork plus rejection, and same-generation restore
separately and prints allocation calls, requested bytes, authoritative
component retention, elapsed nanoseconds, and a semantic checksum.
`EngineCheckpoint` is move-only, so the row explicitly reports
`clone_api=deleted`; the copy measurement covers `CheckpointRetention` plus
execution counters and proves that public accounting projection remains
allocation-free. Fixture
construction is outside every measured region. The accumulated fixture has
deep registered-source and stored-token-list history at command quiescence,
initialized patterns plus journaled mutable exceptions and saved hyphenation
codes, contribution/current/discard page lists, insertions and marks, closed
nested mode history, changed dependency cells, and World effects, an open
stream, and a committed artifact.
...
The executable enforces exact equality of allocation calls and requested bytes
between minimal and accumulated state for capture, mark copy, fork plus
rejection, and restore at each operation count. Real retained payload is
reported separately and is expected to grow. This catches a deep command,
mode, page, mutable hyphenation, World, effect, or artifact copy without
confusing retention with operation cost.

The first `MODE_PAGE_EARLY_SUFFIX_GATE` row captures a legal rootless mode
checkpoint with populated page state, then compares one versus 4,096 later
writes in each growing lane. It isolates the mode/page owner fork, first
destructive mutation, and rejection seams from command, core, and World. The
row reports each exercised destructive family plus the exact accepted
page-journal rewind distance. Rewinding an old page mark is deliberately
proportional to mutations after that mark; it is visible work, not a hidden
capture copy. Mode rewind remains zero-work.

The elapsed nanoseconds are a single-shot diagnostic, not a statistical
threshold. Exact allocation equality, zero-allocation mark copy, and explicit
operation counters are deterministic gates. A stable timing comparison belongs
in a repeated Criterion benchmark, not this correctness executable.
...
The 2026-08-30 run recorded zero allocations and zero requested bytes for both
read depths with checksum `2b890e1a0fde4773`; ordinary and demanded capture
both recorded zero allocations and zero requested bytes at one and 32 closed
mode-history levels.
