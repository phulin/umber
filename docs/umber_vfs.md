...
The TeX adapter checks its rollback-safe pending-output overlay before any VFS
layer or negative binding: same-run generated bytes therefore retain TeX's
close-and-reopen precedence, while an absent pending path still follows the
original probe or required-open intent through the typed resource protocol.
The selected origin remains attached to the resulting `World` input record.
External VFS/distribution reads enter immutable source-closure receipts;
transactional same-run output reads do not, even though both remain available
to lexer provenance and deterministic replay.
