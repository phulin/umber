...
| canonical Story/Gentle | same file: `e2e_conformance_story_canonical` and `e2e_conformance_gentle_canonical` -> `run_plain_fixture_case_canonical` -> `run_file_in_process_canonical`; the latter is also used by seven committed canonical DVI regressions                                                                    | migrated: all nine routes delegate to the same persistent Plain provider path and authenticated identity                                                                  | complete: each route supplies an explicit clock, profile/backend, Nonstop interaction, error widths, finite guards, terminal input, observer, authored root, and typed resources to a provider-owned fresh job; dead private bootstrap helpers remain for the final audit/removal step |
