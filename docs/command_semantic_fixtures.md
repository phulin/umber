...
Beside the committed suite there is a second, generated-on-demand tier of
full-document traces (plain bootstrap, Story, Gentle) under
`tests/corpus/command/tex82-documents`. It uses the identical fixture
contract -- each directory is a real `bootstrap_tex82_fixture` product with a
manifest-bound stream -- but is gitignored and regenerated by
`scripts/build-tex82-document-traces.sh`, because a single document trace runs
17-156 MB. Its identities are pinned in
`tests/tex82-document-trace-manifest.txt` rather than in the regeneration
contract: that tier carries no semantic/audit coverage matrices, since a whole
document is a differential workload rather than a focused, hand-audited
behavior matrix. See "Differential Tracer" in `docs/testing_infrastructure.md`.
...
`tex_exec::CanonicalMainControl`, preserving command-observer ordering while
