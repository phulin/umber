...
The key preimage is unambiguous after its domain prefix. It is, in order:
`umber.format-cache.key\0`, little-endian identity schema `3`, one entry-kind
byte, one engine byte, two zero bytes, little-endian format schema (`u32`), ABI
(`u64`) and lookup (`u64`) fingerprints, four required 32-byte identities
(distribution, format closure, source lock, and build configuration), optional
semantic and producer contract identities, the required 32-byte resource
closure identity, optional generation-guard identity, then the clock's time,
second, day, month, and year as little-endian `i32` values.
Each optional identity is encoded as a one-byte tag (`0` for absent, `1`
followed by the 32-byte fingerprint for present), so absence cannot collide
with an explicitly present zero fingerprint. Image-only CLI entries leave the
three optional identities absent: their source/closure/build inputs and format
ABI already identify the generated bytes, while execution guard limits only
bound construction. Compound fixture entries carry actual semantic, producer,
and guard fingerprints. The schema-3 key misses older cache entries without a
fallback or migration; the entry envelope still validates metadata and payload
under its existing framing version.
...
actual built Umber producer binary and build-script SHA-256 digests, Umber
version, release profile, feature set, and `rustc -Vv` identity. It takes
