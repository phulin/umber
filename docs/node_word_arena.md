...
increments the row and physical incarnations before publishing a new mapping.
When the last logical chunk leaves a physical NodePool slot, its exact 64 KiB
backing remains warm and empty. The flat vectors and backing therefore remain
bounded by maximum simultaneous semantic-owner demand without reallocating the
same superblocks at every page, box, or rollback transition. Checkpoint
retention charges only live backing; vacant reusable capacity is not attributed
to a historical owner. Fork metrics report copied live and vacant rows
...
| `NodeTokenKey`                    |           24 / 4 | Copy-only generation-owned token coordinate; no `Drop`                  |
...
| `Whatsit<GlueSpec, NodeTokenKey>` |           56 / 8 | Mixed; some alternatives own `String`, `Vec`, or `Box`; tokens are keys |
...
| `Mark`         |           31 | class 2 plus copy-only 24-byte `NodeTokenKey` coordinate                         | Class inline and a generation-owned `NodeTokenKey` inline                  |
...
| `DeferredWrite`                                                          |           31 | Copy-only `NodeTokenKey` coordinate            | Inline `NodeTokenKey`                                            |
...
| `DeferredPdfLiteral`                                                     |           31 | Copy-only `NodeTokenKey` coordinate            | Mode in header and inline token key                              |
...
before publication. A vacant physical slot retains an empty `Superblock` for
allocation-free reuse. Reusing an offset in a surviving partial block changes
