...
Schema version participates in both manifest and stream domain hashes. The established v1 observer and decoder remain byte-for-byte compatible and do not accept geometry records; a future v2 observer will select the v2 header/domain before it emits the detached `GeometryEvent` contract. Fixtures are not rewritten in place: a future geometry fixture has a separate v2 manifest and stream, while existing v1 manifests, JSON, headers, and identities remain byte-for-byte valid. The schema is an observation contract only; reference and Umber emitters are introduced independently.
