...
| `CommandStateSnapshot`, `CommandSummary`, tex-state `InputSummary`, and macro-argument checkpoint forms | Clones of the structural transient values already owned by cursors, source pending input, arguments, alignment state, and permitted quiescent builders. | Snapshot clone is the ownership operation. Restore installs the complete old aggregate before the failed aggregate drops. Summary projection preserves roots without upgrading ids. Checkpoint pruning and generation retirement drop the aggregate closure directly.                                                                                                        |
