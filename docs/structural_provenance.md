...
| ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Transient `PackedTokenSpanHandle::Replay` storage                                                    | The exact inserted, converted, recovery, or scanner-replay words and their distinct non-direct roots.                                     | Construction consumes rooted words; push and replacement publish the complete value; delivery borrows the aligned root; cursor retirement, rollback, retry replacement, and final drop release it.                                                                                                                                                         |
| Backed-up `PackedTokenSpanHandle::Replay` storage                                                    | The exact backed-up spelling roots plus independent committed source-range metadata.                                                      | `back_input` moves the delivered command root into the span. `back_list` and e-TeX aftergroup prepend merge structural values before publication. Redelivery borrows the root without lookup; consumption, stack conservation, rollback, and retirement drop it. Rehoming changes only source-range metadata.                                              |
| `ExecutionScratch` macro-word lane, `MacroArguments`, and `MacroArgumentCursor`                      | One generation-owned fixed-chunk lane for all live arguments; compact absolute ranges are nonowning half-open views.                      | Argument collection appends rooted deliveries. Group stripping and delimiter removal adjust only the unpublished range. Activation publication seals the range once; parameter substitution directly indexes the same words; body retirement truncates the reclaimable LIFO suffix. Failed matching and retry truncate or retain the exact pending suffix. |
...
| Live `CommandState`, `CommandSummary`, tex-state `InputSummary`, and macro-argument checkpoint forms | Structural transient values already owned by cursors, source pending input, arguments, alignment state, and permitted quiescent builders. | A resource boundary unwinds ordinary scanner and expansion calls; the host restores the full checkpoint and replays them. Summary projection preserves roots without upgrading ids. Checkpoint pruning and generation retirement drop the durable aggregate closure directly.                                                                              |
...
| Frozen token positions      | `TracedTokenList` carries `OriginListRef`; a stored `PackedTokenSpanHandle` keeps owner identity separate while its immutable packed words retain exact origins.                | Token-list freeze installs token and origin-list coordinates together. Replay borrows the region span; rollback invalidates a discarded list generation before slot reuse.                                                                      |
