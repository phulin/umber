...
Status: accepted design for removing `lopdf` from the PDF test stack.
...
untyped operations. A malformed filtered stream may decode to empty bytes under
Hayro's recovery behavior; retaining the raw bytes and complete decoded digest
...
## Complete current-call-site inventory

The following tables cover every source function whose assertions or fixture
construction currently depend on `lopdf`. Imports and manifest entries carry
no assertions and disappear when these migrations are complete.
