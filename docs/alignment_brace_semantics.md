...
| raw preamble collection                      | retain raw delivery through `#`, `&`, and the terminating `\cr`; at a top-level `&` or `\cr` before a u-template's `#`, TeX82 §760 reports missing parameter and `back_error` restores that delimiter so an empty v-template consumes it as its terminator; freeze u/v pairs, publish preamble completion, then restore the live `aligning` scanner status to normal                | `tex_command::CommandProcessor::begin_alignment_preamble_scan`                                                                        |
...
| `do_endv`                                    | accept only an exhausted active v-template (or its exact driver-carried marker); TeX82 §§343, 765, 772 also permit an exhausted scanner-backed effective `endv` immediately above it                                                                                                                                                                                                | `tex_exec::align::execution`, `CommandState::finish_alignment_cell`                                                                   |
| successful `do_endv`                         | commit the `fin_col` state change, retire any proven exhausted backed-up `endv`, then retire the exact v-template frame before an alias can escape                                                                                                                                                                                                                                  | `CommandState::{alignment_endv_completion,finish_alignment_cell}`                                                                     |
