...
The fixture-only, hermetic correctness tier includes the TeX82 property-catalogue gate in `test-support`. It validates the committed 1,380-module inventory, explicit dispositions, reviewed property shards, canonical citations, ownership, and exact Rust test links without reading or invoking a live reference engine. See [TeX82 Property Catalogue](tex82_property_catalogue.md).
