...
map supply the WOFF2, mapping, provenance, and license records. Root schema 9
...
may publish the manifest-last `manifest-v9.json` root.
...
| `umber-vfs::{ResourceDomain,FileKind,FileRequestKey,FileRequest,ResolvedFile,ProjectWorkspace}`                              | Typed immutable files                                                   | Retain; add distinct missing legacy kinds such as AFM/license where requested and preserve semantic kinds through manifest transport aliases                                                                          |
...
The WASM boundary requires `outputs`. Native CLI and worker resource inputs use
the current typed providers; unknown options are rejected by their ordinary
argument validation. The npm runtime inventory contains
