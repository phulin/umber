...
| `ExecutionScratch` macro words, argument ranges, first-scan facts, and delimiter prefixes             | One `CommandState::scratch`, with a logically contiguous 4,096-word fixed-chunk LIFO lane, one reusable chunk stack, and one live frame-owned metadata stack                          | Match admission takes one absolute lane mark; its direct current-argument slot owns the range and facts while words append. Commit changes only the frame role. A retiring parent transfers its reclaim mark to a pending child; after the last active ancestor retires, rebase physically copies exactly the still-unpublished child suffix and records that count in tests                                                                                                                                                                                                      | Macro input retirement calls `pop_macro_frame`; failed match/discard truncates to its exact inherited mark and returns suffix chunks; retained capacity ends with generation                  | A live frame may cross in-process suspension / never a revision                                       | Heap growth is allowed only at a new simultaneous argument-volume high water; parameter admission resolves and validates one direct absolute range into a first-class cursor, and replay performs no input-stack/activation/range search, packed-span dispatch, chunk-link walk, word copy, per-argument allocation, or owner clone                                 |
...
| Source and input cursor storage                                                                       | `InputState::levels`, `pending_sources`, `SourceLevel`, one `PackedTokenSpanHandle` shape, and one generation-owned segmented replay lane                                             | Input levels and replay entries nest in exact LIFO order; source domains adapt once at level creation, and a tokenizer control-sequence name is call-local until its creating or probing boundary projects one packed identity; delivery writes through the admitted handle into the request's final `CurrentCommand` and advances the fixed frame; error-context traversal projects only the current, budgeted, and bottom displayed levels; file framing travels as the push's call-local name or retirement's existing result bit and renders through the live command context | The transient name ends inside the tokenizer call; EOF/input retirement pops a level and releases its exact span/replay suffix; unopened registrations end when opened or command state drops | Yes / source recipes may detach, live runtime coordinates may not                                     | Replay admission grows only at a new generation high water; warm canonical delivery receives a `TokenWord`, advances one scalar, and performs no allocation, relocation, raw-to-resolved payload copy, generic returned-delivery construction, per-word owner clone, name lookup, creation-policy test, omitted-level pseudoprint, or persistent framing-event poll |
...
owns one caller-provided `Option<CurrentCommand<G>>`; raw input and resolution
write successive phases into that same slot, and nested delivery has its own
slot. The driver returns only a
...
Raw delivery owns no separate value. Stored levels route through the
`PackedTokenSpanHandle` variant chosen at admission, write the raw spelling,
delivery coordinate, optional provenance, and input flags directly into the
request's final `CurrentCommand`, and advance their packed frame in place.
Source levels write the same destination after tokenization. Parameter replay
restarts from the delivered raw word and overwrites that unresolved value on
the next iteration. Meaning resolution then updates the same address before it
is visible to a semantic consumer. The unresolved phase retains no backing
