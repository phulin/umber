...
| Checkpointable page nodes and shipout-derived nodes                                                   | The current generation owns one runtime `PageNodeArena`, cold loaded-node storage, and one physically separate `ShipoutScratchArena<G>`                                               | Page roots rebrand into durable carriers without moving rows; scratch rows nest by scalar marks and retain warmed capacity                                                                                                                                                      | Rootless shipout truncates page suffix above the retained/durable bounds; terminal success/rollback resets scratch wholesale                                                                  | Page/durable roots may cross same-generation checkpoints; scratch ids never cross suspension/revision | Box/page/math/alignment transitions transfer coordinates; shipout borrows graphs in place and constructs only genuinely derived nodes in scratch; no hot graph copy                                                                                                              |
| `ExecutionScratch` macro words, argument ranges, and delimiter prefixes                               | One `CommandState::scratch`, with 4,096-word physical segments, a live absolute-offset bump stack, one sealing lane, fixed macro slots, and a spare-segment pool                      | Macro frames record segment watermarks and up to nine ranges; sealing moves segment owners without copying words, and strict LIFO rewind returns the suffix to reusable high-water storage                                                                                      | Macro input retirement calls `pop_macro_frame`; failed match/discard returns its unpublished segments; retained capacity ends with generation                                                 | A live frame may cross in-process suspension / never a revision                                       | Heap growth is allowed only when the generation reaches a new concurrent high water; `start + position` maps directly to segment/slot, with no per-macro `Vec`, second cursor, relocation, compaction, or payload copy                                                           |
...
| Source and input cursor storage                                                                       | `InputState::levels`, `pending_sources`, `SourceLevel`, one `PackedTokenSpanHandle` shape, and one generation-owned segmented replay lane                                             | Input levels and replay entries nest in exact LIFO order; source domains adapt once at level creation, and a tokenizer control-sequence name is call-local until its creating or probing boundary projects one packed identity; delivery uses `token_at(handle, scalar_offset)` | The transient name ends inside the tokenizer call; EOF/input retirement pops a level and releases its exact span/replay suffix; unopened registrations end when opened or command state drops | Yes / source recipes may detach, live runtime coordinates may not                                     | Replay admission grows only at a new generation high water; warm canonical delivery receives a `TokenWord`, advances one scalar, and performs no allocation, relocation, payload copy, generic delivery construction, per-word owner clone, name lookup, or creation-policy test |
...
| Format image and format construction                                                                  | `DetachedFormatImage` transiently owns bytes plus decoded handle-free rows; admission consumes it into a fresh destination generation                                                 | Cold staging may build several nested DTO tables                                                                                                                                                                                                                                | Admission drops the image before publishing the generation as the initial accepted checkpoint; runtime rows end with that generation                                                          | N/A / yes, via handle-free schema                                                                     | Cold copy/heap allocation and DTO-local relocation are allowed. Live ids, chunks, and generation owners are forbidden on the wire                                                                                                                                                |
...
The frame contains fixed-capacity absolute argument ranges and its opening
segment watermark. Nested macros append to the same `ExecutionScratch` stack;
...
Session admission consumes the detached image and drops its encoded and
decoded payload immediately after successful materialization. It then captures
the materialized generation's pre-job `JobStart` state and publishes that
generation through the ordinary initial accepted-checkpoint slot. The first
document candidate and every later candidate fork that accepted checkpoint
through the same exclusive prior/current path. Rejection leaves the accepted
generation unchanged; acceptance retires it normally. No session, candidate,
checkpoint, or runtime overlay retains the detached image.
...
| Resolved, fact                                                  | Ordinary box, unbox, setbox, page, math, alignment, PDF-form, and node-token lifetime transitions                                                           | Runtime rows remain in the generation page arena and coordinates rebrand or alias; node token fields share immutable stored-token allocations                                                                                                                                                                                                                                                                                                                                           | Warm transfer/alias performs zero allocations and zero physical graph copies; retained checkpoint forks share immutable row allocations                                                                                                                                                                                                                                                                                                                                                                                                                                            | Preserve exact TeX logical copy, consuming transfer, group restore, rollback, and cold-only format materialization; never restore live relocation or token-word rebuilding                                                                                                                               |
...
| 2, fact; resolved                                               | `CommandGenerationOwner` in `crates/tex-command/src/snapshot.rs` and `RetainedCheckpointSlots` in `crates/tex-exec/src/retained_generation.rs`              | The live `CommandStateRoots` is exclusively mutable. Named-boundary publication explicitly clones it once into a private thread-confined `Rc` owner beside its exact attempt mark. Retained-checkpoint aliases share that owner; pruning drops the complete checkpoint and therefore its root owner immediately. The command timeline retains only a monotonic identity serial, and executor checkpoint slots plus their exact live-index backreferences are reused with a fresh serial | The former append-only timeline leak and ordinary mutation admission are closed. Publication pays one coarse root allocation and copies aggregate vectors/scalars without traversing immutable definition, stored-token, node, provenance, or page payload graphs. Warm delivery performs no aggregate clone, `Rc::make_mut`, or atomic ownership admission. Logical retention is proportional to live checkpoints, while physical slot/live/free vectors remain bounded reusable high water proportional to maximum simultaneous retention; stale keys cannot alias a reused slot | Preserve the one explicit cold fork, direct live mutation, owner release, thread confinement, generation-plus-serial validation, and O(live) pruning. Do not scan full slot capacity, relocate a live row, compact coordinates, add atomic live-root ownership, or make the timeline a second root owner |
...
| Resolved, fact                                                  | `DetachedFormatImage { bytes, decoded }` in `crates/tex-state/src/format.rs:215` plus destination staging                                                   | Session admission consumes the validated image, materializes one destination generation, then drops both encoded and decoded payloads before capturing and publishing the initial accepted checkpoint                                                                                                                                                                                                                                                                                   | The format payload has no permanent session owner or runtime overlay. Cold full-buffer decode and destination staging may overlap only inside materialization                                                                                                                                                                                                                                                                                                                                                                                                                      | Preserve immediate post-materialization release and the ordinary two-generation checkpoint path. Streaming decode remains a separate cold-path optimization                                                                                                                                              |
