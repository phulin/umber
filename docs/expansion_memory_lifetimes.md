...
retained header/provenance records. Chunks have neither `Rc` nor an independent
pool/lifetime. A tiny group-local definition therefore uses storage that drops
with its region instead of allocating a mostly empty 16 KiB chunk. Macro
admission resolves the ref/header/start and validates the initial segment once,
then clones that exact owner into a `ResidentMacroBody`. Its flat append-only
overflow directory replaces the former linked directory pages and `OnceCell`
probes. Safe Rust cannot cache a direct chunk borrow beside its owning `Rc`
without a self-reference, raw pointer, or independent chunk owner, so each
delivered word performs one constant-time region-local prefix or directory
slot access. The input row's existing scalar cursor remains the complete
rollback coordinate; its derived segment changes only at the inline boundary
or an explicit 4,096-word overflow crossing.
