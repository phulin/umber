...
| Source and input cursor storage                                                                       | `InputState::levels`, `pending_sources`, `SourceLevel`, one `PackedTokenSpanHandle` shape, and one generation-owned segmented replay lane                                             | Input levels and replay entries nest in exact LIFO order; source domains adapt once at level creation and delivery uses `token_at(handle, scalar_offset)`; retired coarse segments are reusable high water | EOF/input retirement pops a level and releases its exact span/replay suffix; unopened registrations end when opened or command state drops | Yes / source recipes may detach, live runtime coordinates may not                                     | Warm delivery returns canonical `TokenWord`, advances one scalar, and performs no allocation, relocation, payload copy, generic delivery construction, or per-word owner clone |
