...
| Resolved, fact                           | Split `SaveJournal` storage in `crates/tex-state/src/journal.rs`                                                                                                                                | TeX saves occupy reusable per-group segments and retire whole at exit; checkpoint intervals append one reversible first-alternate record per written cell to a typed chunk arena; accepted and candidate suffixes settle by in-place swaps; nested operation attempts retain only their ordered suffix until commit or rollback                                                                                                                                                                                    | The former 112-byte mixed generation-long row and 56 MiB vector high water are removed. Dense reads remain unchanged, fixed checkpoint marks copy no values, stable checkpoint cursors pin only legitimate open-group segments, and the ordinary level-zero policy leaves no closed group history retained                                                                                                                                                                                       | Preserve direct reads, reverse rollback, packed entries, O(1) append, whole-chunk fork settlement, whole-segment retirement, and profiling of live/spare capacity; do not add overlays, densification, compaction, forwarding, or per-entry ownership                                                                                                                                                                                            |
...
| Resolved, fact                           | Superseded or rollback-abandoned durable immutable values and core/node checkpoint banks                                                                                                        | Definition/token-list carriers own private shared payloads and release them on last drop. A named checkpoint owns one exact `StateCore` and page-node bank; a candidate moves that bank out of its singular slot, appends only private dense/publisher/node suffixes, and either returns it after exact rollback or consumes it on acceptance. Glue/provenance retain direct-index rows with bounded publisher cursors. The completed primitive registry is one immutable shared root                              | Candidate fork, restore, reject, accept, and first mutation allocate independently of accumulated dense writes, save history, glue/provenance rows, node coordinates, and primitive rows. Rejection/retry reproduces exact row ids. There is no ordinary-read overlay, first-write COW, per-value owner, prefix replay, or coordinate relocation                                                                                                                                                 | Preserve named-boundary bank ownership, direct dense reads, one coarse lineage authority, exact private-suffix journals/cursors, and branch-local node generation frontiers. Publication cost and retained-bank accounting belong to the checkpoint-retention work; do not move them back into candidate construction                                                                                                                            |
| 2, fact; resolved                        | `CommandGenerationOwner`, `CommandTimeline`, and `LogicalStack` in `crates/tex-command/src/{snapshot,timeline}.rs` plus the parked command slot in `crates/tex-exec/src/retained_generation.rs` | One `CommandState` owns a typed chunk pool and fork arena. Named-boundary publication seals a move-only frame with fixed logical-stack and journal marks. The retained generation parks that sole physical command owner while a checkpoint is idle; candidate creation moves it into the current borrower, detaches the prior suffix, and rejection or acceptance settles that suffix in place. Input, parameter, condition, group, aftergroup, and alignment payloads stay in append storage after a logical pop | Warmed capture, same-generation restore, command fork, first mutation, and fork-plus-first-mutation copy zero accumulated payload. Stable physical-owner marks survive repeated acceptance, stale suffix marks fail closed, and suspension keeps the typed command owner reachable through the retained-generation sidecar. No checkpoint carries an aliasing owner, and no prefix is cloned, split, collected, or replayed outside the explicit edit transition                                 | Preserve direct mutation, fixed sealed marks, thread confinement, move-only owner handoff, reverse undo/forward redo, and the prior/current lineage bound. Do not add an aggregate-root clone, shared mutable tail, first-write COW, compactor, root registry, per-frame heap owner, ordinary-path atomic admission, or a third lineage                                                                                                          |
| Resolved, fact                           | Coarse `ModeNestStorage` and `PageBuilderState` timelines in `tex-exec` and `tex-state`                                                                                                         | Named boundaries retain owner-relative cursor/frame marks. Mode levels and node sequences remain in one loaned coarse store; page contribution/current/discard sequences, insertion state, and marks remain in one runtime page store. Candidate fork moves each coarse store into the current slot and rejection returns it after rollback                                                                                                                                                                        | Checkpoint clone aliases only the mode owner and copies fixed runtime/page marks. Capture and fork do not clone accumulated mode levels, mode lists, page lists, insertions, or marks. Publication charges the generation-owned command/mode/page/World/hyphenation/PDF/dependency/source/font/core storage once, fixed marks and counters per restart root, and detached boundary evidence separately. The budget releases roots at publication rather than hiding a larger pre-acceptance peak | Preserve short typed mode read/write guards, prior/current-only store loans, rollback before returning a rejected loan, one-way shipout detachment, and flat accumulated-state allocation gates. Optional convergence identity must use complete journal-maintained component roots and fail closed while any root is absent. Do not expose `RefCell`, add ordinary-path COW, per-node owners, compaction, root registration, or a third lineage |
