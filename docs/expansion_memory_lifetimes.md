...
| Source and input cursor storage                                                                       | `InputState::levels`, `pending_sources`, `SourceLevel`, compact `TokenPayload` coordinates, and one generation-owned segmented replay lane                                            | Input levels and replay entries nest in exact LIFO order; a tokenizer control-sequence name is call-local until projected to one packed identity; retired coarse segments are reusable high water | The transient name ends inside the tokenizer call; EOF/input retirement pops a level and releases its replay suffix; unopened registered sources end when opened or command state drops | Yes / source recipes may detach, live runtime coordinates may not                                     | Replay admission grows only at a new generation high water; production source delivery copies only packed token identity and provenance across the tokenizer seam; ordinary push/pop does not allocate, relocate, compact, or copy a whole payload |
