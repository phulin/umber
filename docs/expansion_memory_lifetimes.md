...
| Current candidate generation                                                                          | `RevisionCandidate::generation` owns one independent `RetainedEngineGeneration` after execution begins                                                                                | Exactly one session-issued candidate lease; storage is not reused as another generation               | Candidate rejection/drop, or move into the prior slot on acceptance                                                             | Yes / becomes the new prior                                                               | Heap arenas are allowed; no live relocation, rehome, or cross-generation id copy                                                                                  |
...
The implemented invariant is exactly two slots, not "two plus any number of
...
existing session state performs only an `Arc` retain and allocates no heap.
...
instead of preserving it for rejection. The coarse session-boundary lease is
therefore the required interior owner; it is not a per-generation row, a
runtime registry, or an ordinary-read lookup.
...
The remaining verified architectural inconsistency with the largest retention
risk is the append-only command timeline after checkpoint pruning. Exclusive
candidate issuance is implemented and the former unbounded multi-candidate
finding is resolved. The format closure and abandoned durable rows are verified
over-retention, but their real workload cost remains a measurement question.
No compactor, forwarding scheme, or reachability registry is an acceptable
correction.
