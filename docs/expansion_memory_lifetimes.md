...
| Macro definitions                                                                                     | One immutable format region, one revision-global region, or a nested forked local-group region; eqtb/save lanes hold compact keys                                                     | Local group segments nest; aliases copy only keys; active local macro input rows hold coarse region leases                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Checkpoint truncation, normal whole-group retirement after meanings restore, or generation retirement; a live local input row delays only its whole region                                    | Yes / no runtime key crosses revision                                                                 | Ordinary def scans once into final transactional storage and let copies no token payload. A local-to-global let exceptionally promotes one span once; no definition has a per-value heap owner                                                                                                                                                                                                                                 |
...
`DefinitionArena` instead owns packed headers and words in the immutable
format, revision-global, and nested forked local-group regions. A compact key
names one header/span; a borrowed view exposes it without ownership or a word
copy. Local input rows may pin one complete group region, while checkpoint and
generation owners govern global suffixes. Accounting follows suffix truncation
...
`DefinitionId<G>` key and borrowed `DefinitionView<'_, G>` surface. Eqtb and
...
`DefinitionId<G>` key into the final `CurrentCommand`; it acquires no definition
...
known. The current macro activation/input row holds the compact definition key,
frame id, and the optional coarse local-region lease. Its replacement input
carries only that activation identity and length; delivery and cold context
rendering borrow parameter and replacement spans through the key without a
per-definition owner. The replacement input retires only after all argument
...
During collection, the scanner-owned `TokenCollector` holds the direct lane
cursor, delimiter-prefix coordinates, brace depth, and first-scan facts; it
publishes that argument once when it
...
owned; ordinary definitions write semantic words directly into the selected
transactional definition region. Standalone escaping general text instead owns one
