...
| Macro definitions                                                                                     | Every live semantic carrier owns one private generation-branded non-atomic owner of the builder-filled allocation                                                                     | Aliases clone explicitly; moves transfer owners                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Last eqtb, journal, input/expansion, checkpoint, PDF, continuation, or view owner drop                                                                                                        | Yes / no runtime handle crosses revision                                                              | Publication retains the checked builder allocation without traversing or copying its words. Aliasing, reads, moves, restore, and warmed attempt reuse allocate nothing                                                                                                                                                                                                                                    |
...
`DefinitionArena` similarly publishes the same allocation that an
attempt-local definition builder fills and retains no body row. Its non-atomic
owner does not repeat the serial, parameter split, parsed parameter program,
token words, or accounting capability: those values occur once in the shared
allocation and the final semantic owner drop performs exact accounting
release. A stored token-list view, cursor, macro
...
An expanded macro definition is the exact boundary where chain adoption is
not valid: its final owner is a provenance-free semantic definition builder,
so child traced words are converted once into that builder before durable
publication.
...
- No duplicate durable token-list publication beside a macro definition. One
  checked builder allocation becomes the durable definition allocation; final
  publication may validate and retain it but may not traverse or copy its
  words.
