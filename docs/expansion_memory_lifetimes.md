...
| 2, fact; resolved                        | `CommandGenerationOwner`, `CommandTimeline`, and `LogicalStack` in `crates/tex-command/src/{snapshot,timeline}.rs` plus `RetainedCheckpointSlots` in `crates/tex-exec/src/retained_generation.rs` | The live `CommandStateRoots` is the sole mutable borrower. Named-boundary publication retains one reusable timeline frame containing fixed logical tops and compact undo coordinates; checkpoint aliases increment that frame's owner count. Input, parameter, condition, group, aftergroup, and alignment payloads remain in generation-owned append storage after a logical pop. Main-control drop returns the root loan before fork, rejection restores the accepted marks and drops the current suffix, and a later fork moves the returned root into the sole current borrower | Warmed capture, retained-owner clone, same-generation restore, command fork, first mutation, and fork-plus-first-mutation allocate and copy zero accumulated payload at both one and 64 command units. Reusable frame metadata remains bounded by simultaneous retention, stale serials cannot alias reused slots, and exact rollback preserves overwritten reachable rows through compact undo entries                                                                                          | Preserve direct mutation, thread confinement, generation-plus-serial validation, move-only root handoff, and the prior/current lineage bound. Do not add an aggregate-root clone, first-write COW, compactor, root registry, per-entry heap owner, ordinary-path atomic admission, or a third lineage                                                                                                                                            |
