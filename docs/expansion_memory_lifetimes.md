...
| 2, fact; resolved                                               | `CommandGenerationOwner` in `crates/tex-command/src/snapshot.rs` and `RetainedCheckpointSlots` in `crates/tex-exec/src/retained_generation.rs`              | Each snapshot owner directly retains its aggregate `CommandStateRoots` through a private thread-confined `Rc` plus its exact attempt mark. Pruning drops the complete checkpoint and therefore its root owner immediately; the command timeline retains only a monotonic identity serial. Executor checkpoint slots and their exact live-index backreferences are reused with a fresh serial | The former append-only timeline leak is closed. Capture remains O(1); only the first post-capture mutation can clone the coarse root; warmed delivery performs neither an aggregate clone nor atomic ownership admission. Logical retention is proportional to live checkpoints, while physical slot/live/free vectors remain bounded reusable high water proportional to maximum simultaneous retention; stale keys cannot alias a reused slot | Preserve direct owner release, thread confinement, generation-plus-serial validation, and O(live) pruning. Do not scan full slot capacity, relocate a live row, compact coordinates, add atomic command-root ownership, or make the timeline a second root owner |
