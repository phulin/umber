...
| Transitional attempt/scanner storage                                                                  | One `CommandAttempt` owns `AttemptArena` vectors, scoped rows, reusable definition builders, and recycled token-buffer `Vec`s                                                         | Child scopes and scanners nest; buffers and definition-word capacity are reused after truncation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Commit/rollback or cancellation truncates to the opening mark; physical capacities end with `CommandState`/generation                                                                         | Yes, because `PendingCommandAttempt` owns the exact attempt row / no                                  | `\def`, `\edef`, `\read`, and `\readline` keep one semantic definition row rather than duplicate token lists. The row never enters checkpoint state; successful publication traverses it once into the final shared definition                                                                                                                      |
