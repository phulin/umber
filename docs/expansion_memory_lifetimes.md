...
- **Implemented now** is verified in the source at commit `41dd95dc7`.
...
generation** is the complete live storage bundle for one accepted or candidate
revision. A **durable value** is immutable data which may be named by live TeX
...
process-immutable tables and compiled semantics
  `-- session + append-only interning epoch
        +-- prior accepted generation (optional, read-only)
        `-- current candidate generation (exclusive in the target model)
...
accept candidate:
  validate and quiesce -> retire old prior wholesale -> candidate becomes prior

reject or cancel candidate:
  drop its continuations -> drop command/executor roots -> retire generation
```

The target therefore has at most two live revision generations: the optional
...
| Prior accepted generation                                                                             | `Session::prior_generation` owns one `RetainedEngineGeneration`                                                                                                                       | Not nested and not reusable as candidate scratch                                                      | Explicit retirement immediately before installing the accepted candidate, or session drop                                       | N/A / exactly one acceptance boundary                                                     | Whole-owner retention is allowed; rows are not copied into the candidate                                                                                          |
| Current candidate generation                                                                          | `RevisionCandidate::generation` owns one independent `RetainedEngineGeneration` after execution begins                                                                                | Target: one exclusive candidate; storage is not reused as another generation                          | Candidate rejection/drop, or move into the prior slot on acceptance                                                             | Yes / becomes the new prior                                                               | Heap arenas are allowed; no live relocation, rehome, or cross-generation id copy                                                                                  |
...
| Macro definitions                                                                                     | `DefinitionArena`: append-only definition rows plus a contiguous token-word vector                                                                                                    | Rows are not nested or reused after publication                                                       | Whole generation retirement                                                                                                     | Yes / no runtime id crosses revision                                                      | Current final allocation copies/streams words into the arena; target hot construction is destination-directed and has no temporary semantic-word vector           |
| Token lists used by toks registers, token parameters, marks, hooks, and stored replay                 | `TokenListArena`: dense sealed rows plus 64-word `TokenChunk`s                                                                                                                        | Builders may nest; discarded unpublished chains and builder slots are reusable; sealed chains are not | Discard recycles an unpublished chain; every sealed row and its chunks end with generation                                      | Yes / no runtime id crosses revision                                                      | Direct builder append is implemented. The cold slice wrapper may read an existing slice; migrated hot scanners must append directly without an intermediate `Vec` |
| Glue and provenance rows                                                                              | Append-only generation `GlueArena` and `ProvenanceArena` vectors                                                                                                                      | Not nested and not reused after publication                                                           | Whole generation retirement                                                                                                     | Yes / only after explicit handle-free detachment                                          | Small value copy and arena growth are allowed; no per-value owner                                                                                                 |
...
`SessionInternerEpoch` is the one intentional exception to revision-local
ownership. Its `Interner` holds an append-only string arena, entry vector, and
hash buckets. Symbols remain stable through edits, so group exit, operation
rollback, and candidate rejection do not remove names. Explicit budgets cap
names, slots, and bytes. Retiring the epoch releases the tables. A symbol is an
id, not an owner.

### Prior and current generations

The **target invariant** is exactly two slots, not "two plus any number of
candidates callers happen to retain." The prior slot is optional and
read-only. The current candidate has the only execution lease. Acceptance
first proves that scratch and typed operations are quiescent, prunes optional
checkpoint roots, retires the old prior wholesale, and moves the candidate
owner into the prior slot. Rejection drops the candidate owner wholesale.

The implementation already retires the old prior in this order, but the public
`Session::start_*_candidate` methods take `&self`. Several independently owned
`RevisionCandidate`s can therefore coexist and later allocate generations.
That is a verified target violation, recorded in the retention audit below.

Runtime ids are invariantly branded by a generation. Copying an id is cheap
and allowed inside an admitted generation; it never extends lifetime. A coarse
`GenerationOwner` extends the whole generation. No individual token list,
definition, chunk, input frame, or group owns an `Arc`.
...
Consequently every chunk in that sealed list's chain stays present until the
generation is retired, even if the current register has since been overwritten.
Recycling one such chunk would silently change any cursor or older restored
value that still addresses it.

This is also the rule for any definition representation composed from fixed
backing chunks: **all chunks which make up a live definition stay owned by the
one generation**. Chunks do not have independent history or retirement. The
current `DefinitionArena` has not made that cutover: it owns definition rows
and one contiguous parameter/replacement word vector, separate from
`TokenListArena`. The document therefore does not claim that current macro
definition text is chunked.
...
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1, fact                                  | `DefinitionArena`, sealed `TokenListArena` rows/chunks, `GlueArena`, and `ProvenanceArena` in `crates/tex-state/src/{definition_arena,durable_arena}.rs`    | Publishing a row retains it until whole generation retirement                                                                                                                                                           | Intended append-only generation ownership. Growth is proportional to all published immutable values in the generation, not just currently reachable values                                          | Keep wholesale drop; prevent avoidable publication and use direct final builders                                                                                                                                        |
...
| 3, fact                                  | Superseded or rollback-abandoned durable definition/token/glue/provenance rows                                                                              | A durable row can be published and then lose every live state/input/checkpoint root through overwrite, group restoration, or later operation failure; no value-arena cursor truncates it                                | The unreachable row/chunks remain until generation retirement. This is compatible with wholesale arena drop but is avoidable generation bloat; impact is proportional to abandoned publications     | Delay sealing/publication until commit, use operation-owned unpublished builders, and transfer only validated final roots. Do not add liveness tracing or compaction                                                    |
| 3 and 4, fact                            | `CommandTimeline.rows: Vec<CommandTimelineRow>` in `crates/tex-command/src/snapshot.rs:25`                                                                  | Every snapshot appends an `Arc<CommandStateRoots>`. `RetainedEngineGeneration::prune_checkpoints` clears checkpoint sidecar slots, but no path removes the matching timeline row; all rows release only with generation | Pruned/unreachable checkpoint roots stay live. This defeats the checkpoint budget for command-root payload and grows per named/transient snapshot, potentially without a document-independent bound | Make the retained checkpoint store the sole root owner or use reusable bounded timeline slots with serial validation; release a row when its last checkpoint/transient cursor is consumed, without relocating live rows |
...
| 4, fact                                  | Public candidate creation in `crates/tex-incr/src/lib.rs:1531` and `:1624`                                                                                  | `start_*_candidate(&self)` can return many candidates; each can be driven until it owns a distinct `RetainedEngineGeneration`; only dropping/accepting each candidate releases it                                       | Violates exactly prior+current. Caller-retained candidate memory can grow with the number of live candidates times a full generation                                                                | Add a unique session-issued current-candidate lease/typestate, returned on accept/reject/drop, and reject creation while one candidate exists                                                                           |
...
The two verified architectural inconsistencies with the largest retention risk
are the append-only command timeline after checkpoint pruning and the ability
to drive multiple candidate generations. The format closure and abandoned
durable rows are verified over-retention, but their real workload cost remains
a measurement question. No compactor, forwarding scheme, or reachability
registry is an acceptable correction.
