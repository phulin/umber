...
old tree's complete hashed inventory. Reference runners materialize each archive
inside a fresh temporary directory, so generated `.aux`, `.log`, `.fls`, PDF,
and EPS-conversion outputs cannot become source inputs on later runs.
...
may therefore be absent without changing corpus identity or weakening path,
type, byte-length, or SHA-256 verification.
