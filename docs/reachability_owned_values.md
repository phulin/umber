...
| Macro definition   | Env meaning cells; one admitted chunk closure for active macro calls                                                           | Env undo entries; command summaries; Universe snapshots; generation forks; format bases; patches               |
...
| Command live state                    | `SourceLevel::every_eof`, `TokenPayload::Stored`, alignment cell u/v templates, admitted macro chunks, queued named-token-list observations, and completed scanner/request values that cross the processor borrow.                 | Command operation snapshots and `CommandSummary` clones share roots. Macro replacement/argument payloads retain only coordinates into the admitted macro and command argument chunks. Input retirement, failure rollback, and retry restore them. |
...
| Active macro delivery        | `push_macro_activation` admitted chunk and replacement/argument coordinates                                                                            | Admission retains the live token/provenance closure once for up to 64 definitions; the replacement cursor and activation contain only chunk/span coordinates.                                                | Macro-body retirement removes the copy-only activation. Later redefinition publishes another chunk snapshot, so an active replacement remains exact.    |
...
| Active macro delivery                | Admission retains one immutable macro-chunk closure; activation and body cursor publish only definition/chunk/span coordinates. Later assignment of the control sequence changes neither that chunk snapshot nor its origins. | A focused redefinition test activates one replacement, redefines the macro, and observes the original replacement from the already-active cursor.                                                                                                  |
...
- `MacroMeaning`'s parameter/replacement coordinate. A live definition body
  owns both children before command admission; the handle-free packed cache
  stores only ids, and one admitted chunk closure retains the corresponding
  children and provenance during active replay.
...
| Active command invocation     | `ParameterState` owns one admitted macro-chunk root closure; each 48-byte activation stores only definition, argument, and invocation coordinates. | Admission collects the live token/provenance closure once per 64-record chunk. Replacement and parameter levels borrow packed spans; retirement removes only copyable coordinates.                    |
