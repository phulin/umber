...
- Editor source fragments use a session-scoped append-only `FragmentStore`
  ...
  ordinary engine snapshots remain O(1), and only the validated fragment
  facade can append registrations.
...
| `FragmentId`, fragment `SourcePos` range | `FragmentStore` plus `EditorLayout` | session-dense fragment id and globally disjoint non-reused logical range | direct origins retain the same low 31-bit position encoding; layout pieces retain opaque fragment ids and validated byte ranges | none; editor layouts are session-local diagnostic state | O(1) `Arc` fragment-table snapshot; layouts are immutable per `LayoutGeneration` | fragment rows/ranges are append-only across fork discard; a missing current piece resolves as typed `Deleted`, never an aliased offset | validated `FragmentStore::append`, `EditorLayout::new`, and `ProvenanceResolver::resolve_layout_origin` |
