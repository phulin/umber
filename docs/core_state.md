...
- Ordinary one-scalar backed source tokens encode their starting `SourcePos`
  ...
  production inputs.
- Editor source fragments use a session-scoped append-only `FragmentStore`
  over the same non-rewinding logical allocator as rollback-coupled engine
  regions. Immutable `EditorLayout` piece tables map fragment positions into
  one accepted document generation; the layout-aware resolver returns current
  document offsets, typed deletion metadata, foreign engine provenance, or
  unknown without rewriting retained origins. Fragment-store clones and
  ordinary engine snapshots remain O(1); each clone receives a fresh append
  lineage embedded in newly minted `FragmentId`s, so equal dense suffix slots
  in sibling stores cannot alias. Aggregate installation requires and
  revalidates the paired layout, and only the validated fragment facade can
...
| `FragmentId`, fragment `SourcePos` range | `FragmentStore` plus `EditorLayout` | append-lineage tag plus session-dense slot and globally disjoint non-reused logical range | direct origins retain the same low 31-bit position encoding; layout pieces retain opaque tagged fragment ids and validated byte ranges | none; editor layouts are session-local diagnostic state | O(1) persistent metadata-root snapshot with a fresh append lineage; O(log n) metadata append/lookup; mutable bytes remain session-owned; layouts are immutable per `LayoutGeneration` | inherited ids remain valid; sibling suffix appends are foreign even at an equal slot; fragment rows/ranges are append-only across fork discard; a missing current piece resolves as typed `Deleted`, never an aliased offset | validated `FragmentStore::append`, `EditorLayout::new`, aggregate paired installation, and `ProvenanceResolver::resolve_layout_origin` |
