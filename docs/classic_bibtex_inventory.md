...
`umber2-ild0.12` added backend-neutral `LatexProjectOptions` and
`BibliographyProjectOptions`. The project session accepts explicit biblatex,
...
The WebAssembly boundary continues to accept the original biblatex-only option
shape by translating an omitted `mode` into the unified native configuration.
...
Changing the bibliography policy resets the backend session and removes
...
| --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unified API accepts biblatex and classic modes                              | `LatexProjectOptions`/`BibliographyProjectOptions` and project tests in `crates/umber/src/latex_project/tests.rs`                                           |
| Explicit and auto selection                                                 | `classic_projects_converge_transactionally_with_explicit_and_auto_modes`                                                                                    |
| Native/WASM result bindings                                                 | native project tests plus `project_binding_accepts_classic_bibliography_options`                                                                            |
