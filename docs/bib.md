...
modules and audits that the census has no ignore, expected-failure, or
unexpected-pass allowance. Each module retains its complete pinned upstream
source plus the exact actual and expected expressions. Subprocess-driven
upstream output cases are expectations on an in-process session's exit status,
generated bytes, and rendered diagnostics. Assertion order, fixture
references, expected bytes, normalization forms, and Unicode therefore remain
directly auditable without a file-wide compatibility escape hatch.
...
test. The integration scaffold fixes both the 51-module and 1,275-assertion
census and rejects compatibility ignore, expected-failure, and
unexpected-pass markers, so no file-wide wrapper, skip, or development gate
can hide an assertion.
