...
backend.
The supported LaTeX-DVI contract uses the complete pdfTeX engine in DVI mode;
`\pdftexversion` and the other pdfTeX primitives remain defined when
`\pdfoutput=0` (pdftex.web §§236, 413). Plain e-TeX 2.6 defines neither
...
`\pdfstartlink`, `\pdfendlink`, `\pdfoutline`, `\pdfdest`, `\pdfobj`, and
`\pdfrefobj` are covered alongside the remaining families documented below.
...
the deferred fallback-font name.
...
pdftex.web §§1524, 1556, and 1563 make `\pdfoutline` an any-mode immediate
document mutation. The canonical command path scans the optional expanded
attribute, shared typed action, optional signed count, and expanded title
before allocating the action, item, and title identities in source order.
...
pdftex.web §1544 orders `\pdfrefobj`'s PDF-output check before its integer
scan, object-existence validation, whatsit allocation, list mutation, and
object-number assignment. A DVI failure therefore preserves the integer
