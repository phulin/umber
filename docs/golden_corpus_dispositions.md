...
| `tex_exec`    | Crate-internal preserved reference observations, owned by `tex-exec` and the pdfTeX adapter in `umber` | `tex82_reference_observation_fixtures_match_canonical_execution`, twelve named pdfTeX unit tests, and `retained_pdftex_extension_fixtures_compare_oracle_projections` | Retain all thirty-seven exact source/reference cases. Seventeen TeX82 cases actively execute their sources to bounded clean completion and compare explicit ordered terminal/log projections against the preserved observation. All twenty pdfTeX cases have property IDs, pinned source citations, exact active consumers, and explicit status/terminal/log dispositions in the pdfTeX extension catalogue; known mismatches remain strict bug-linked xfails. |
...
`tests/pdftex-properties/catalogue.json`; none is inventory-only evidence. The
three `.30`-blocked cases are exactly the retained
