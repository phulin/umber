...
behind `umber2-alfh.29` and `umber2-alfh.30`.
...
| `tex_exec`    | Crate-internal preserved reference observations, owned by `tex-exec` and the pdfTeX adapter in `umber` | `tex82_reference_observation_fixtures_match_canonical_execution` plus twelve named pdfTeX unit tests | Retain all thirty-seven exact source/reference cases. Seventeen TeX82 cases actively execute their sources to bounded clean completion and compare explicit ordered terminal/log projections against the preserved observation. Twelve pdfTeX cases have existing literal consumers. The remaining eight are retained-but-blocked on the extension property catalogue and runner tracked by `umber2-alfh.29`; inventory is not claimed as behavioral evidence. |
